...
export function SectionSelector({ value, onChange }: { value: SectionRecord | null; onChange: (section: SectionRecord | null) => void; }) {
  ...
  return (
    <section className="panel sectionSelectorPanel">
      <span className="eyebrow">Approved CISC data</span>
      <h2>W-section</h2>
      <button type="button" className="sectionPickerButton" onClick={() => setOpen(true)} disabled={loading}>
        <span><strong>{value?.designation ?? (loading ? "Loading…" : "Select W-section")}</strong><small>{value?.designation_imperial ?? ""}</small></span><b>⌄</b>
      </button>
      {error && <p className="error">{error}</p>}
      {open && <div className="sectionModalBackdrop" onClick={() => setOpen(false)}>
        <div className="sectionModal" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="sectionModalList">
            {filtered.map((section) => <button type="button" className={value?.id === section.id ? "sectionChoice active" : "sectionChoice"} key={section.id} onClick={() => { onChange(section); setOpen(false); }}><strong>{section.designation}</strong><span>{section.designation_imperial ? `${section.designation_imperial} · ` : ""}{section.source}</span></button>)}
          </div>
        </div>
      </div>}
    </section>
